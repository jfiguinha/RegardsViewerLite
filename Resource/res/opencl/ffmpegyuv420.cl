...
//----------------------------------------------------
__kernel void BicubicNV12towxWidget(__global float4 *output, const __global uchar *inputY, const __global uchar *inputUV, int widthIn, int heightIn, int widthOut, int heightOut, int flipH, int flipV, int angle, int bicubic) 
{ 
	int width = widthOut;
	...
	float4 color;
	if(bicubic)
		color = BiCubicNV12(posX, posY, inputY, inputUV, widthIn, heightIn);
	else
		color = GetColorFromNV12(inputY, inputUV, posX,  posY, widthIn, heightIn);
