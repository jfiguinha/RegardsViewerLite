...
bool CSqlPhotoGPS:: InsertPhoto(const int &numPhoto, const wxString & filepath, const int & numFolderId)
{
	...
	fullpath.Replace("'", "''");
	return (ExecuteRequestWithNoResult("INSERT INTO PHOTOSGPS (NumPhoto, FullPath, NumFolderId) VALUES (" + to_string(numPhoto) + ", '" + fullpath + "'," + to_string(numFolderId) + ")") != -1) ? true : false;
}

bool CSqlPhotoGPS::DeletePhoto(const int64_t &numPhoto)
{
	return (ExecuteRequestWithNoResult("DELETE FROM PHOTOSGPS WHERE NumPhoto = " + to_string(numPhoto)) != -1) ? true : false;
}
