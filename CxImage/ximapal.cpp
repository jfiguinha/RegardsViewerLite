...
void CxImage::SetPalette(uint32_t n, uint8_t *r, uint8_t *g, uint8_t *b)
{
	...
	if (!b) b = g;
	RGBQUAD* ppal=GetPalette();
	uint32_t m=min((uint32_t)n,(uint32_t)head.biClrUsed);
...
void CxImage::SetPalette(rgb_color *rgb,uint32_t nColors)
{
	...
	RGBQUAD* ppal=GetPalette();
	uint32_t m= min((uint32_t)nColors,(uint32_t)head.biClrUsed);
	for (uint32_t i=0; i<m;i++){
...
void CxImage::SetPalette(RGBQUAD* pPal,uint32_t nColors)
{
	...
	memcpy(GetPalette(),pPal, min((uint32_t)GetPaletteSize(),(uint32_t)(nColors*sizeof(RGBQUAD))));
	info.last_c_isvalid = false;
}
////////////////////////////////////////////////////////////////////////////////
/**
...
void CxImage::SetClrImportant(uint32_t ncolors)
{
	...
	case 1:
		head.biClrImportant = min((uint32_t)ncolors,(uint32_t)2);
		break;
	case 4:
		head.biClrImportant = min((uint32_t)ncolors,(uint32_t)16);
		break;
	case 8:
